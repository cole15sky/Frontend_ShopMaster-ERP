...
import { User, Mail, Phone, Briefcase, Shield, Camera } from "lucide-react";
      ...
      <div className="max-w-2xl space-y-6">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          ...
          <div className="flex items-center gap-5 mb-8">
            ...
            <div>
              <p className="text-xl font-bold">{profile.full_name || "Staff Member"}</p>
              <p className="text-slate-400 text-sm">{profile.email}</p>
              <span className={`inline-block mt-2 px-3 py-1 text-xs rounded-full border font-medium ${ROLE_BADGE[profile.role] || ""}`}>
                {profile.role}
              </span>
            </div>
          </div>
